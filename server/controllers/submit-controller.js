...
module.exports = {
  async index(ctx) {
    ...
    if (
      !(strapi.config.get("plugin.ezforms.captchaProvider.name") === "none") &&
      strapi.config.get("plugin.ezforms.captchaProvider.name")
    ) {
      verification = await strapi
        .plugin("ezforms")
        .service(strapi.config.get("plugin.ezforms.captchaProvider.name"))
      ...
      if (!verification.valid) {
        ...
        if (verification.code === 500) {
          ctx.internalServerError(
            (errorMessage =
              "There was an error, check Strapi logs for more details. " +
